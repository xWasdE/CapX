document.addEventListener('DOMContentLoaded', () => {
    ...
    function applyCalcToScenario(type, scenarioId) {
    ...
    function handleSmartCalc(type) {
        const target = targetScenarioSelector.value;
        if (!target) {
        ...
        let success = false;
        if (target === 'all') {
            const allScenarioCards = scenariosContainer.querySelectorAll('.calculator-card');
            if (allScenarioCards.length === 0) {
                 alert("Hesaplama yapılacak dengeleyici senaryo bulunmuyor.");
                 return;
            }
            allScenarioCards.forEach(card => {
                const scenarioId = card.dataset.scenarioId;
                if (applyCalcToScenario(type, scenarioId)) {
                    success = true;
                }
            });
        } else {
            success = applyCalcToScenario(type, target);
        }

        if (success) {
            calculate();
        } else {
            alert("Hesaplama için koşullar sağlanamadı. Lütfen Ana Yatırım Tutarını ve ilgili senaryo oranlarını kontrol edin.");
        }
    }
    ...
    function loadFromURL() {
        ...
        // "Tüm Dengeleyiciler" seçeneği sabit olduğu için, sadece dinamik olanları temizle
        targetScenarioSelector.innerHTML = '<option value="all">Tüm Dengeleyiciler</option>';
        scenarioCounter = 0;
